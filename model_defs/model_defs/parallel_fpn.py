...
class ParallelFPN(nn.Module):
    ...
    def __init__(self, base_features, growth_rate, downchanneling_factor, dropout_p=0, drop_path_p=0):
        ...
        self.interp_r_2 = nn.Sequential(
            ...
            # nnblock.get_norm_layer(norm_type="gn",num_channels=dcf(1)),
            nnblock.SEBlock3d(dcf(1)),
            nnblock.get_norm_layer(norm_type="gn",num_channels=dcf(1))
        )
        ...
        self.head = nn.Sequential(
            ...
            # nnblock.get_norm_layer("gn", total_concat_channels),
            nn.SiLU(inplace=True),
            nnblock.SEBlock3d(channels = total_concat_channels),#BLACK MAGIC WTF
            # SE Block Mystery (15+ ablations, no satisfying explanation):
            # - SE blocks accelerate convergence (epoch to 0.96: 60→15) despite learning near-identity weights (Δweight < 1e-3 from init) 
            # - SE with frozen weights >> literal x*0.5 in same position (unexplained forward/backward equivalence break)
            #also tried 0.5 lr to mimic SE's 0.5 gradient scaling from the chanel weighting
            # - Placement critical: (GN → SiLU → SE → raw Conv) >> (SE → PreActBlock with skip)
            # - Skip connections after SE kill performance (mechanism unclear)
            # - Zero-init ≈ random-init performance (confirms FC path doesn't matter)
        
            # - Architecture: GN → SiLU → SE → raw Conv3d empirically optimal, theoretical basis unknown
...
class ParallelFPNLightHead(nn.Module):
    """ParallelFPN variant with lightweight head (no res blocks, just conv-norm-silu-conv)."""
    ...
    def __init__(self, base_features, growth_rate, downchanneling_factor, dropout_p=0, drop_path_p=0):
        ...
        downchannel_progression = [dcf(i) for i in [1, 2, 3, 4, 5]]
        total_concat_channels = sum(downchannel_progression)
        print(f'ParallelFPNLightHead: total concat features: {total_concat_channels}')

        # Light head: dropout -> silu -> SE -> 3x3 conv -> norm -> silu -> 1x1 conv
        head_intermediate = power_2_rounding(total_concat_channels / 4)
        self.head = nn.Sequential(
            nn.Dropout3d(p=self.dropout_p),
            nn.SiLU(inplace=True),
            nnblock.SEBlock3d(channels=total_concat_channels),
            nn.Conv3d(total_concat_channels, head_intermediate, kernel_size=3, padding=1, bias=False),
            nnblock.get_norm_layer("gn", head_intermediate),
            nn.SiLU(inplace=True),
            nn.Conv3d(head_intermediate, 1, kernel_size=1, bias=True)
        )
...
if __name__ == '__main__':
    model = ParallelFPN(base_features=4, growth_rate=2.3, downchanneling_factor=2)
    model.print_params()