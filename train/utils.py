...
class Logger:
    ...
    def log_training_settings(self,model, save_dir):

        architecture_path = inspect.getfile(type(model))
        shutil.copy(architecture_path, save_dir)
        
        train_path = Path.cwd() / 'train/train.py'#save da train script
        shutil.copy(train_path, save_dir)
