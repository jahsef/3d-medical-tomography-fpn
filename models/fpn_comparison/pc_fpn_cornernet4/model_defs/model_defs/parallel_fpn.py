...
class ParallelFPN(nn.Module):
    ...
    def __init__(self, base_features, growth_rate, downchanneling_factor, dropout_p=0, drop_path_p=0):
        ...
        self.head = nn.Sequential(
            ...
            #   - ~15+ experiments
            #   - SE vs no SE vs ×0.5 (theoretically SE is equivalent to 0.5 channel attention when frozen but even frozen beat out 0.5 scalar mult)
            # and even when unfrozen SE learned in the range of 1e-3 max weight difference
            #   - Skip vs no skip 
            # (SE,preactblock) vs (norm,act,SE,conv) somehow SE here just outperforms skip by a lot, 
            #0.5 scalar is also poo
            #   - Different LRs 
            #   - Different placements
            #   - Zero-init vs random-init (zero init should theoretically provide )
            #   Conclusion: GN → SiLU → SE → raw Conv3d works best for reasons we can't explain. SE does something beyond 0.5 scaling even with frozen weights.
