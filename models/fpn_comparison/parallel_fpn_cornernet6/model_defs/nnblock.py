...
class PreActResBlock3d(nn.Module):
    def __init__(self, in_channels, out_channels,norm_type, stride=1, kernel_size=3, target_channels_per_group=4, drop_path_p=0):
...
class PreActRefinementBlock3d(nn.Module):
    def __init__(self, in_channels, norm_type,kernel_size=3, drop_path_p=0):
        super().__init__()
        fart = kernel_size - 1
        padding = fart//2
...
class PreActResBottleneckBlock3d(nn.Module):
    def __init__(self, in_channels, out_channels, norm_type,stride=1, target_channels_per_group=4, bottleneck_ratio=4):
...
class PreActGroupPointBlock3d(nn.Module):
    def __init__(self, in_channels, out_channels, groups, norm_type):
        super().__init__()
        self.features = nn.Sequential(
            get_norm_layer(norm_type, in_channels),
            nn.SiLU(inplace=True),
...
class PreActDownchannel3d(nn.Module):
    def __init__(self, in_channels, out_channels, norm_type, drop_path_p=0):
...
class SEBlock3d(nn.Module):
    def __init__(self, channels, reduction=4):
        ...
        #init so that sigmoid is near saturated and our channel weights are around 1.0
        nn.init.zeros_(self.fc[-1].weight)
        nn.init.constant_(self.fc[-1].bias, 4.0)  # sigmoid(4) ≈ 0.98 ≈ 1.0
