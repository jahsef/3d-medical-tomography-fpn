...
class MotorIdentifier(nn.Module):
    ...
    def __init__(self, dropout_p=0, drop_path_p = 0, norm_type="gn"):
        ...
        DEEP_EXTRA_DCF = 2#deep extra channel downchanneling factor
        print(f'F_I, feature_progressioon, DC: {F_I}, {FEATURE_PROGRESSION}, {DC}')
        ...
        self.interp_r_2 = nn.Sequential(
            nnblock.PreActDownchannel3d(growth_fn(1), growth_fn(1)//DC, drop_path_p = drop_path_p),
            nnblock.SEBlock3d(growth_fn(1)//DC, reduction=4),
            nnblock.get_norm_layer("gn", growth_fn(1)//DC),
            nn.SiLU(inplace=True),
        )
        self.interp_r_4 = nn.Sequential(
            nnblock.PreActDownchannel3d(growth_fn(2), growth_fn(2)//DC, drop_path_p = drop_path_p),
            nnblock.SEBlock3d(growth_fn(2)//DC, reduction=4),
            nnblock.get_norm_layer("gn", growth_fn(2)//DC),
            nn.SiLU(inplace=True),
        )
        self.interp_r_8 = nn.Sequential(
            nnblock.PreActDownchannel3d(growth_fn(3), growth_fn(3)//DC, drop_path_p = drop_path_p),
            nnblock.SEBlock3d(growth_fn(3)//DC, reduction=4),
            nnblock.get_norm_layer("gn", growth_fn(3)//DC),
            nn.SiLU(inplace=True),
        )
        self.interp_r_16 = nn.Sequential(
            nnblock.PreActDownchannel3d(growth_fn(4), growth_fn(4)//(DC*DEEP_EXTRA_DCF), drop_path_p = drop_path_p),
            nnblock.SEBlock3d(growth_fn(4)//(DC*DEEP_EXTRA_DCF), reduction=4),
            nnblock.get_norm_layer("gn", growth_fn(4)//(DC*DEEP_EXTRA_DCF)),
            nn.SiLU(inplace=True),
        )
        self.interp_r_32 = nn.Sequential(
            nnblock.PreActDownchannel3d(growth_fn(5), growth_fn(5)//(DC*DEEP_EXTRA_DCF), drop_path_p = drop_path_p),
            nnblock.SEBlock3d(growth_fn(5)//(DC*DEEP_EXTRA_DCF), reduction=4),
        ...
        self.head = nn.Sequential(
            nn.Dropout3d(p=self.dropout_p),
            nn.Conv3d(total_concat_channels, total_concat_channels//2, kernel_size=1),
            nnblock.get_norm_layer("gn", total_concat_channels//2),#some of these are hardcoded because of gn stuff
            nn.SiLU(inplace=True),
            nnblock.PreActResBlock3d(total_concat_channels//2,total_concat_channels//4,kernel_size=3, drop_path_p = drop_path_p),
            nnblock.PreActResBlock3d(total_concat_channels//4,total_concat_channels//8,kernel_size=3, drop_path_p = drop_path_p),
            nnblock.get_norm_layer("gn", total_concat_channels//8),
            nn.SiLU(inplace=True),
            nn.Conv3d(total_concat_channels//8, 1, kernel_size=1)
        )
