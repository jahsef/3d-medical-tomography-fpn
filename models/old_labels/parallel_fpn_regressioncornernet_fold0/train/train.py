...
from loss import WeightedBCELoss, BCEFocalLoss, AdaptedCornerNetLoss, FuzzyCornerNetLoss, CombinedFocalLoss, RegressionCornerNetLoss
...
if __name__ == "__main__":
    ...
    CONFIG = {
        ...
        # Loss
        'loss_function': 'regressioncornernet',  # 'bce', 'mse', 'bcefocal', 'weightedbce', 'adaptedcornernet', 'fuzzycornernet'
        ...
        # Paths
        'save_dir': './models/old_labels/parallel_fpn_regressioncornernet_fold0',  
        'exist_ok':False,
    ...
    #     print(f"Using mae focal loss with gamma={CONFIG['gamma']}")
        ...
        conf_loss_fn = CombinedFocalLoss(alpha=CONFIG['alpha'], beta=CONFIG['beta'])
        print(f"Using CombinedFocal loss with alpha={CONFIG['alpha']}, beta={CONFIG['beta']}")
    elif CONFIG['loss_function'] == 'regressioncornernet':
        conf_loss_fn = RegressionCornerNetLoss(pos_threshold=CONFIG['pos_threshold'], alpha=CONFIG['alpha'], beta=CONFIG['beta'])
        print(f"Using RegressionCornerNet loss with alpha={CONFIG['alpha']}, beta={CONFIG['beta']}")
    else:
        raise ValueError(f"Unknown loss function: {CONFIG['loss_function']}. Choose from: 'bce', 'mse', 'bcefocal', 'weightedbce', 'adaptedcornernet', 'fuzzycornernet', 'combined', 'regressioncornernet'")
