...
class CombinedFocalLoss(nn.Module):
    def __init__(self, alpha=2.0, beta=4.0):
        """Combined focal loss - works like BCE focal but crushes FP especially.

        Args:
            alpha: focal power for error weighting (default 2)
            beta: background suppression power (default 4)
        """
        super().__init__()
        self.alpha = alpha
        self.beta = beta
    ...
    def forward(self, inputs, targets):
        ...
        error = torch.abs(targets - pred)
        # First term: error-weighted log penalty
        # Second term: background suppression with FP crushing
        loss = -(((targets - pred) ** self.alpha) * torch.log(1 - error) +
                 ((1 - targets) ** self.beta) * (pred ** self.alpha) * torch.log(1 - pred))
