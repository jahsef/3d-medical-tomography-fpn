...
def get_tomo_folds():
    """Load tomo_id -> fold mapping from RELABELED_DATA.csv"""
    csv_path = Path(__file__).parent.parent / 'data/original_data/RELABELED_DATA.csv'
    df = pd.read_csv(csv_path)
    return df.groupby('tomo_id')['fold'].first().to_dict()

def _log(message, level, print_function_name:bool):
...
def load_ground_truth(csv_path, tomo_ids, downsampling_factor):
    """
    Load ground truth motor locations from either old or new label format.
    Returns dict of tomo_id -> list of [z, y, x] coords in downsampled space.
    """
    import ast
    df = pd.read_csv(csv_path)
    ...
    for tomo_id in tomo_ids:
        ...
        if is_new_format:
            # New format: one row per tomo, coordinates is normalized list string
            if len(tomo_rows) == 0:
                gt_dict[tomo_id] = []
                continue
            row = tomo_rows.iloc[0]
            coords_str = row['coordinates']
            coords_list = ast.literal_eval(coords_str) if coords_str and coords_str != '[]' else []
            shape = (row['z_shape'], row['y_shape'], row['x_shape'])
            for norm_coord in coords_list:
                # Denormalize and downsample
                motors.append([
                    norm_coord[0] * shape[0] / downsampling_factor,
                    norm_coord[1] * shape[1] / downsampling_factor,
                    norm_coord[2] * shape[2] / downsampling_factor
                ])
        else:
            # Old format: one row per motor, Motor axis columns are pixel coords
