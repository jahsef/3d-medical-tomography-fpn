...
class MotorIdentifier(nn.Module):
    ...
    def __init__(self,dropout_p = 0, norm_type = "gn"):
        ...
        self.enc_4 = nn.Sequential(
            nnblock.PreActResBlock3d(F_I*2, F_I*3, stride = 2, kernel_size=3, norm_type=norm_type),
        )
        #1/8
        self.enc_8 = nn.Sequential(
            nnblock.PreActResBlock3d(F_I*3, F_I*4, stride = 2, kernel_size=3, norm_type=norm_type),
        )
        #1/16
        self.enc_16 = nn.Sequential(
            nnblock.PreActResBlock3d(F_I*4, F_I*6, stride = 2, kernel_size=3, norm_type=norm_type),
        )
        #1/32
        self.enc_32 = nn.Sequential(
            nnblock.PreActResBlock3d(F_I*6, F_I*8, stride = 2, kernel_size=3, norm_type=norm_type),
        )
        
        
        self.fpn_2_block = nnblock.FPNBlock3d(F_I*2, F_I*2)
        self.fpn_2_conv = nn.Conv3d(F_I*2, F_I*2, kernel_size=1)
        
        self.fpn_4_block = nnblock.FPNBlock3d(F_I*3, F_I*3)
        self.fpn_4_conv = nn.Conv3d(F_I*3, F_I*3, kernel_size=1)
        
        self.fpn_8_block = nnblock.FPNBlock3d(F_I*4, F_I*4)
        self.fpn_8_conv = nn.Conv3d(F_I*4, F_I*4, kernel_size=1)
        
        self.fpn_32_block = nnblock.FPNBlock3d(F_I*8, F_I*8)
        self.fpn_32_conv = nn.Conv3d(F_I*8, F_I*8, kernel_size=1)
        
        
        total_concat_channels = F_I*(2 + 3 + 4 + 8 + 6 + 8)
        
        self.head = nn.Sequential(
            nn.Dropout3d(p = self.dropout_p),
            nnblock.PreActResBlock3d(total_concat_channels,total_concat_channels//4,kernel_size=1,norm_type="gn"),
            ...
            nnblock.get_norm_layer(norm_type="gn", num_channels=total_concat_channels//16),
            nn.SiLU(inplace=True),
            nn.Conv3d(total_concat_channels//16, 1, kernel_size= 1, stride= 1)
        )
    ...
    def forward(self, x):
        ...
        # Process each FPN block with target_size
        ...
        upsample_enc_32_x = F.interpolate(enc_32_x, size=target_size, mode='trilinear')
        fpn_concat = torch.cat([fpn_2_x, fpn_4_x, fpn_8_x, enc_16_x, fpn_32_x, upsample_enc_32_x], dim=1)
    ...
    def print_params(self):
        ...
        fpn_2_params = (sum(p.numel() for p in self.fpn_2_block.parameters() if p.requires_grad) +
                        sum(p.numel() for p in self.fpn_2_conv.parameters() if p.requires_grad))
        fpn_4_params = (sum(p.numel() for p in self.fpn_4_block.parameters() if p.requires_grad) +
                    sum(p.numel() for p in self.fpn_4_conv.parameters() if p.requires_grad))
        fpn_8_params = (sum(p.numel() for p in self.fpn_8_block.parameters() if p.requires_grad) +
                    sum(p.numel() for p in self.fpn_8_conv.parameters() if p.requires_grad))
        fpn_32_params = (sum(p.numel() for p in self.fpn_32_block.parameters() if p.requires_grad) +
                        sum(p.numel() for p in self.fpn_32_conv.parameters() if p.requires_grad))
        # Head (unchanged)
        ...
        print(f'  - fpn_2: {fpn_2_params:,} (block: {sum(p.numel() for p in self.fpn_2_block.parameters() if p.requires_grad):,}, conv: {sum(p.numel() for p in self.fpn_2_conv.parameters() if p.requires_grad):,})')
        print(f'  - fpn_4: {fpn_4_params:,} (block: {sum(p.numel() for p in self.fpn_4_block.parameters() if p.requires_grad):,}, conv: {sum(p.numel() for p in self.fpn_4_conv.parameters() if p.requires_grad):,})')
        print(f'  - fpn_8: {fpn_8_params:,} (block: {sum(p.numel() for p in self.fpn_8_block.parameters() if p.requires_grad):,}, conv: {sum(p.numel() for p in self.fpn_8_conv.parameters() if p.requires_grad):,})')
        print(f'  - fpn_32: {fpn_32_params:,} (block: {sum(p.numel() for p in self.fpn_32_block.parameters() if p.requires_grad):,}, conv: {sum(p.numel() for p in self.fpn_32_conv.parameters() if p.requires_grad):,})')
