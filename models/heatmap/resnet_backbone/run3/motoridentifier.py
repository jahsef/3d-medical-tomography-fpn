...
class MotorIdentifier(nn.Module):
    def __init__(self, dropout_p = 0, norm_type = "gn"):
        ...
        nn.SiLU(),
        nn.Conv3d(F_I*2, 40, kernel_size=1)
        )
        ...
        nn.SiLU(),
        nn.Conv3d(F_I*4, 40, kernel_size=1)
        )
        ...
        nn.SiLU(),
        nn.Conv3d(F_I*8, 40, kernel_size=1)
        )
        ...
        nn.SiLU(),
        nn.Conv3d(F_I*16, 40, kernel_size=1)
        )
        ...
        nn.SiLU(),
        nn.Conv3d(F_I*32, 40, kernel_size=1)
        )

        total_concat_channels = 40 * 6  # 192 channels total

        self.head = nn.Sequential(
            nn.Dropout3d(p=self.dropout_p),
            nnblock.PreActResBlock3d(total_concat_channels, total_concat_channels//3, kernel_size=3, norm_type="gn"),
            nnblock.PreActResBlock3d(total_concat_channels//3, total_concat_channels//6, kernel_size=3, norm_type="gn"),
