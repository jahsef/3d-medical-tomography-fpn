...
class MotorIdentifier(nn.Module):
    ...
    def __init__(self,max_motors:int):
        ...
        self.encoder = nn.Sequential(
            # Input: 1 channel, output: FOC//4 channels
            nn.Conv3d(1, FOC//4, kernel_size=5, stride=1, padding=2),
            # 32^3 resolution
            nnblock.PreActResBlock3d(FOC//4, FOC//4, stride=1, kernel_size=3),
            nnblock.PreActResBlock3d(FOC//4, FOC//2, stride=2, kernel_size=3),
            # 16^3 resolution  
            nnblock.PreActResBlock3d(FOC//2, FOC//2, stride=1, kernel_size=3),
        ...
        self.decoder = nn.Sequential(
            # 16^3 -> 32^3 resolution
            nnblock.UpsamplePreActResBlock3d(FOC, FOC//2, stride=2, kernel_size=3),
            # Feature refinement
            nnblock.PreActResBlock3d(FOC//2, FOC//2, stride=1, kernel_size=3),
            # 32^3 -> 64^3 resolution
            nnblock.UpsamplePreActResBlock3d(FOC//2, FOC//4, stride=2, kernel_size=3),
        )

        # Classification head
        self.classification_head = nn.Sequential(
            nnblock.PreActResBlock3d(FOC//4, FOC//8, stride=1, kernel_size=3),
            nn.BatchNorm3d(FOC//8),
            nn.SiLU(inplace=True),
            nn.Conv3d(FOC//8, 1, kernel_size=1, bias=False)  # Fixed: was FOC//16, now FOC//8
        )
