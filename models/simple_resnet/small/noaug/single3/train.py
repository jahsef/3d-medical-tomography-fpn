...
class ContinuousFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        bce_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction='none')
        # Weight by target importance AND prediction difficulty
        target_weight = targets ** self.gamma  # Focus on high-value targets
        pred_weight = (1 - torch.sigmoid(inputs)) ** self.gamma  # Focus on hard predictions
        focal_weight = self.alpha * target_weight * pred_weight
...
if __name__ == "__main__":
    ...
    save_dir = './models/simple_resnet/small/noaug/single3/'
    ...
    # conf_loss_fn = WeightedBCELoss(pos_weight=100)
