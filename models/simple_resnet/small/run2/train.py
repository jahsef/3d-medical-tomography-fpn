...
if __name__ == "__main__":
    ...
    train_transform = monai.transforms.Compose([
        ...
        transforms.RandGaussianNoised(keys='patch', dtype=torch.float16, prob=0.3, std=0.1),
        transforms.RandShiftIntensityd(keys='patch', offsets=0.1, safe=True, prob=0.3),
        transforms.RandAdjustContrastd(keys="patch", gamma=(0.9, 1.1), prob=0.3),
        transforms.RandScaleIntensityd(keys="patch", factors=0.1, prob=0.25),
        
        # Spatial augmentations (keeping these efficient)
        transforms.RandRotate90d(keys=["patch", "label"], prob=0.4),
        ...
        # Stronger spatial deformations
        transforms.RandRotated(keys=['patch', 'label'], range_x=0.25, range_y=0.25, range_z=0.25, prob=0.125, mode='trilinear'),
        transforms.RandZoomd(keys=['patch', 'label'], min_zoom=0.9, max_zoom=1.1, prob=0.125, mode='trilinear'),
    ...
    model.load_state_dict(torch.load(r'C:\Users\kevin\Documents\GitHub\kaggle-byu-bacteria-motor-comp\models\simple_resnet/small/run1\best.pt'))
    ...
    save_dir = './models/simple_resnet/small/run2'
    ...
    warmup_steps = int(0.0 * total_steps)#% of steps warmup, 5% is about 2 epochs
