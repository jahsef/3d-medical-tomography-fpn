...
if __name__ == "__main__":
    ...
    train_transform = monai.transforms.Compose([
        ...
        transforms.RandScaleIntensityd(keys="patch", factors=0.1, prob=0.3),
        
        # Spatial augmentations (keeping these efficient)
        transforms.RandRotate90d(keys=["patch", "label"], prob=0.7, spatial_axes=[1,2]),
        transforms.RandFlipd(keys=['patch', 'label'], prob=0.7, spatial_axis=[0,1,2]),
        ...
        transforms.RandRotated(keys=['patch', 'label'], range_x=0.25, range_y=0.25, range_z=0.25, prob=0.15, mode='trilinear'),
        transforms.RandZoomd(keys=['patch', 'label'], min_zoom = 0.9, max_zoom = 1.1, prob = 0.15, mode = 'trilinear'),
        
        transforms.RandCoarseDropoutd(#basically just noise at these settings
            keys="patch", 
            holes=5, 
            spatial_size=(5, 10, 10), 
            prob=0.5
        ),
    ...
    # model.load_state_dict(torch.load(r'C:\Users\kevin\Documents\GitHub\kaggle-byu-bacteria-motor-comp\models\simple_resnet/med/run1\best.pt'))
    ...
    save_dir = './models/simple_resnet/med/run2'
