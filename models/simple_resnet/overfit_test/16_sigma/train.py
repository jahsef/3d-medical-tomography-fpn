...
if __name__ == "__main__":
    ...
    train_transform = monai.transforms.Compose([
        ...
        # transforms.RandGaussianNoised(keys='patch', dtype=torch.float16, prob=0.325, std=0.1),
        # transforms.RandShiftIntensityd(keys='patch', offsets=0.1, safe=True, prob=0.325),
        # transforms.RandAdjustContrastd(keys="patch", gamma=(0.9, 1.1), prob=0.325),
        # transforms.RandScaleIntensityd(keys="patch", factors=0.1, prob=0.325),
        
        # Spatial augmentations (keeping these efficient)
        # transforms.RandRotate90d(keys=["patch", "label"], prob=0.9, spatial_axes=[1,2]),
        # transforms.RandFlipd(keys=['patch', 'label'], prob=0.9, spatial_axis=[0,1,2]),
        
        # transforms.SpatialPadd(keys=['patch', 'label'], spatial_size=[172,320,320], mode='reflect'),
        # transforms.RandSpatialCropd(keys=['patch', 'label'], roi_size=[160,288,288], random_center=True),

        # transforms.RandRotated(keys=['patch', 'label'], range_x=0.33, range_y=0.33, range_z=0.33, prob=0.3, mode=['trilinear', 'nearest']),
        # transforms.RandZoomd(keys=['patch', 'label'], min_zoom = 0.9, max_zoom = 1.1, prob = 0.3, mode = ['trilinear', 'nearest']),
        
        # transforms.RandCoarseDropoutd(#basically just noise at these settings
        #     keys="patch", 
        #     holes=8, 
        #     spatial_size=(7, 14, 14), 
        #     prob=0.5
        # ),
    ...
    # model.load_state_dict(torch.load(r'C:\Users\kevin\Documents\GitHub\kaggle-byu-bacteria-motor-comp\models\simple_resnet/med/run2\best.pt'))


    save_dir = './models/simple_resnet/overfit_test/16_sigma'
    ...
    blob_sigma = 16 #this is in real coord space not downsampled
    #gaussian edge weighting basically
    weight_sigma_scale = 1 #larger is prolly fine for downscaled heatmaps
    ...
    #     r'C:\Users\kevin\Documents\GitHub\kaggle-byu-bacteria-motor-comp\models\simple_resnet/small/run1\best_optimizer.pt', 
    #     map_location=device
    # )
    ...
    # conf_loss_fn = BCETopKLoss(k = 1200)
